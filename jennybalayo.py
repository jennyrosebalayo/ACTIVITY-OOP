...
def submit():
    conn=sqlite3.connect('C:/Users/PC/OneDrive/Documents/selffdata.db')
    c=conn.cursor()
    c.execute("INSERT INTO selffdata VALUES(:f_name,:L_name,:age,:address,:email)",
              {
                  'f_name':f_name.get(),
                  'L_name':L_name.get(),
                  'age':age.get(),
...
def query():
        conn=sqlite3.connect('C:/Users/PC/OneDrive/Documents/selffdata.db')
        c=conn.cursor()
        c.execute("SELECT *,oid FROM datafelff")
        records=c.fetchall()
...
'''
c.execute("""CREATE TABLE "datafelff" (
	"f_name"	TEXT,
	"l_name"	TEXT,
	"age"	INTEGER,
	"address"	TEXT,
...
def edit():
    ...
    c.execute("SELECT * FROM datafelff WHERE oid=?", (record_id,))
    record = c.fetchone()
    ...
    def save_update():
        ...
        c.execute('''UPDATE datafelff SET
...
def submit():
    ...
    c.execute("INSERT INTO datafelff VALUES (:f_name, :l_name, :age, :address, :email)",
              {
                'f_name': f_name.get(),
                'l_name': l_name.get(),
                'age': age.get(),
...
def delete():
    conn = sqlite3.connect('C:/Users/PC/OneDrive/Documents/selffdata.db')
    c = conn.cursor()
    c.execute("DELETE FROM datafelff WHERE oid=?", (delete_box.get(),))
    conn.commit()
